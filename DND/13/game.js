(() => {
  ...
  function createPlayerActs(acts) {
    ...
    return acts
      ...
      .map((act) => ({
        ...
        baseDamageBonus: Number.isFinite(act.baseDamageBonus)
          ? act.baseDamageBonus
          : Number.isFinite(act.damageMultiplier)
            ? Math.max(0, act.damageMultiplier - 1)
            : 0,
        damageResistance: Number.isFinite(act.damageResistance) ? clamp(act.damageResistance, 0, 1) : 0,
  ...
  function rollSableDisabledCommandsForRound() {
    const distortion = state.sableScreenDistortion;
    if (!isSableEncounter() || !distortion.postFinale) {
      state.sableDisabledCommands = state.party.map(() => null);
      return;
    }

    state.sableDisabledCommands = state.party.map(() =>
      Math.floor(Math.random() * menuItems.length)
    );
    state.sableCommandGlitchRound++;
  }

  function sableCommandDisabled(partyIndex, commandIndex) {
    return (
      isSableEncounter() &&
      state.sableScreenDistortion.postFinale &&
      state.sableDisabledCommands[partyIndex] === commandIndex
    );
  }
  ...
  function playerDamageMultiplier(player) {
    ...
    const permanentBaseBonus = Number.isFinite(player?.permanentBaseDamageBonus)
      ...
      : 0;
    const rageBaseBonus = player?.rageActive && Number.isFinite(player.rageBaseDamageBonus)
      ? player.rageBaseDamageBonus
      : 0;
    const temporaryMultiplier = Number.isFinite(player?.temporaryDamageMultiplier)
      ? player.temporaryDamageMultiplier
      : 1;

    return (baseMultiplier + permanentBaseBonus + rageBaseBonus) * temporaryMultiplier;
  }
  ...
  function beginNextDamageSpell() {
    ...
    if (actor) {
      const requestedRole = action.act.sprite || "action";
      actor.actionSpriteRole = actor.spriteKeys[requestedRole] ? requestedRole : "action";
      if (action.act.script === "recklessAttack") actor.recklessVulnerable = true;
    }
  }
  ...
  function beginNextPersistentEffect() {
    ...
    if (effect.script === "attackOfOpportunity" && effect.pendingHits <= 0) {
      beginNextPersistentEffect();
      return;
    }
    ...
    persistent.hitCount = effect.script === "attackOfOpportunity" ? effect.pendingHits : 0;
  ...
  function updatePersistentEffect() {
    ...
    if (effect.script === "attackOfOpportunity") {
  ...
  function damageRandomLivingPlayer(amount) {
    ...
    const interceptor = living.find((player) => player.intercepting);
    ...
    const resistanceMultiplier = target.rageActive ? 1 - target.rageResistance : 1;
    const recklessMultiplier = target.recklessVulnerable ? 2 : 1;
    const damage = baseDamage > 0
      ? Math.max(1, Math.round(baseDamage * resistanceMultiplier * recklessMultiplier))
      : 0;
  ...
  function recordPartyStrike() {
    for (const effect of state.persistentEffects) {
      if (effect.script !== "attackOfOpportunity") continue;
  ...
  function resetDownedPlayerEffects(player) {
    ...
    removeStarryFormsForPlayer(player);
  }
  ...
  function canAffordAct(act) {
    ...
    if (act.effect === "rage" && actor?.rageActive) return false;
    ...
    if (act.requiresAllPartyAlive && livingPartyMembers().length !== state.party.length) return false;
  ...
  function selectNextTurn() {
    ...
    for (let offset = 1; offset <= turns.length; offset++) {
      ...
      unlockSableScreenDistortion(index);
      intensifySableScreenDistortion(index, turns.length, rawTurn?.postFinaleTrigger === true);
  ...
  function beginMenu(message, { resetCommands = true } = {}) {
    ...
    if (resetCommands) {
      ...
      resetPartyCommands();
      rollSableDisabledCommandsForRound();
    }
  }
  ...
  function beginEnemyAttack() {
    ...
    state.soul.invuln = 0;
    state.grazeGlow = 0;
    state.sableInvertControls = false;
  ...
  function useMenuSelection() {
    ...
    if (sableCommandDisabled(actingIndex, state.selected)) {
  ...
  function stopSableSystemGlitching() {
    const distortion = state.sableScreenDistortion;
  ...
  function beginTeamMercyResolution() {
    const bossData = currentBossData();
    stopSableSystemGlitching();
    state.bullets = [];
    state.phase = PHASE.ENEMY_DIALOG;
    state.box = { ...BOX_RECT.TEXT };
    state.message = bossData.mercySuccess || "* The party cleanses the system.";
    state.textTimer = 0;
    state.enemyDialogMessage = bossData.systemPatchEnemyDialog || "...";
    state.enemyDialogTimer = 0;
    ...
    state.enemyDialogOnComplete = () => {
      ...
      state.mercy.fadeDuration = Number.isFinite(bossData.systemPatchMercyFadeDuration)
  ...
  function beginDefeatDissolve() {
    ...
    state.enemyDialogMessage = bossData.defeatDialog || enemyData.defeatDialog || "...";
    state.enemyDialogTimer = 0;
    dissolve.timer = 0;
  ...
  function updateEnemyAttack() {
    ...
    if (state.enemyTimer >= state.enemyDuration && state.phase === PHASE.ENEMY) {
      if (isSableEncounter() && state.currentTurn?.index === 9) {
        unlockSableScreenDistortion(10);
      }
      state.bullets = [];
  ...
  function moveSoul() {
    ...
    const box = state.box;
    const movementSpeed = soul.speed * (state.sableMovementSpeedMultiplier ?? 1);
  ...
  function soulInputWithDelay() {
    ...
    const delay = Math.max(0, Math.floor(state.sableInputDelayFrames || 0));
    if (delay === 0) return current;

    const buffer = state.sableInputDelayBuffer || (state.sableInputDelayBuffer = []);
    buffer.push(current);
  ...
  function updateGreenShieldDirection() {
    ...
    const applyDirection = (direction) => {
      if (state.sableShieldInputPhase === "left") {
        state.shieldDirection = ({ up: "left", left: "down", down: "right", right: "up" })[direction];
      } else {
        state.shieldDirection = direction;
      }
    };
  ...
  function partySelectionCardRect(index) {
    const rosterSize = Math.max(1, Array.isArray(window.PLAYER_DATA) ? window.PLAYER_DATA.length : 1);
    const gap = rosterSize > 4 ? 10 : 14;
    const outerInset = rosterSize > 4 ? 32 : 52;
    const w = (W - outerInset * 2 - gap * (rosterSize - 1)) / rosterSize;
  ...
  function resetGame() {
    ...
    state.consumedTurns.clear();
    state.seenTurns.clear();
    state.sableScreenDistortion.unlocked = false;
    state.sableScreenDistortion.postFinale = false;
    state.sableScreenDistortion.timer = 0;
    state.sableScreenDistortion.nextBurst = 0;
    state.sableScreenDistortion.burstUntil = -1;
    state.sableScreenDistortion.seed = 0;
    state.sableScreenDistortion.nextSpriteFlicker = 0;
    state.sableScreenDistortion.spriteFlickerStart = -1;
    state.sableScreenDistortion.spriteFlickerUntil = -1;
    state.sableScreenDistortion.spriteFlickerX = 0;
    state.sableScreenDistortion.spriteFlickerY = 0;
    state.sableDisabledCommands = [];
    state.sableCommandGlitchRound = 0;
    state.sableAssetMismatchActive = false;
    state.sableAssetMismatchSwitchFrames = 0;
    state.sableShieldGlitchOut = null;
    state.sableFinaleGreenSides = null;
  ...
  function unlockSableScreenDistortion(turnIndex) {
    if (!isSableEncounter() || turnIndex < 10 || state.sableScreenDistortion.unlocked) return;
  ...
  function intensifySableScreenDistortion(turnIndex, turnCount, explicitFinaleTrigger = false) {
    if (
      !isSableEncounter() ||
      !Number.isInteger(turnCount) ||
      turnCount <= 0 ||
      (!explicitFinaleTrigger && turnIndex !== turnCount - 1) ||
      state.sableScreenDistortion.postFinale
    ) return;

    const distortion = state.sableScreenDistortion;
    if (explicitFinaleTrigger) state.enemyPostFinaleReached = true;
  ...
  function updateSableScreenDistortion() {
    const distortion = state.sableScreenDistortion;
  ...
  function sableDistortionBurstActive() {
    const distortion = state.sableScreenDistortion;
    return distortion.unlocked && distortion.timer <= distortion.burstUntil;
  }

  function sableSpriteFlickerNoise(cycle, offset) {
    const distortionSeed = state.sableScreenDistortion.unlocked
      ? state.sableScreenDistortion.seed
      : 0;
    const raw = Math.sin(
      (cycle + 1) * 12.9898 +
      offset * 78.233 +
  ...
  function drawSableSpriteFlicker(renderCtx, enemySprite, x, y, size, spriteKey) {
    ...
    const intensified = state.sableScreenDistortion.unlocked;
    const postFinale = state.sableScreenDistortion.postFinale;
  ...
  function drawSableDistortionSlices(regions, sliceCount, maxShift, seedOffset) {
    ...
    sableDistortionCtx.drawImage(canvas, 0, 0);
  ...
  function drawEnemy() {
    ...
    const diagonalFlicker = sableDiagonalSpriteFlickerOffset();
    ...
    drawSableSpriteFlicker(ctx, enemySprite, spriteX, spriteTop, spriteSize, transitionVisual.spriteKey);
  ...
  function drawPartySprite(player, x, y) {
    const size = 78;
    const rageIdleRole = player.rageActive && player.spriteKeys.rage ? "rage" : "default";
    const idleRole = player.temporaryDamageBuffTurns > 0 ? "starshot" : rageIdleRole;
    let actionRole = player.actionSpriteRole;

    if (player.rageActive && actionRole === "item" && player.spriteKeys["rage-item"]) {
      actionRole = "rage-item";
    } else if (player.rageActive && actionRole === "attack" && player.spriteKeys["rage-attack"]) {
      actionRole = "rage-attack";
    }

    const role = player.hp > 0 ? actionRole || idleRole : "down";
  ...
  function drawCommandIcons(card) {
    for (let i = 0; i < menuItems.length; i++) {
      ...
      const disabled = sableCommandDisabled(state.partyTurnIndex, i);
  ...
  function drawPersistentEffectAction() {
    ...
    if (effect.script === "starryFormArcher") {
      ...
      effect.script !== "starryFormChalice" &&
      effect.script !== "attackOfOpportunity" &&
      persistent.timer <= 30
    ) {
      const progress = easeInOutCubic(clamp(persistent.timer / 30, 0, 1));
    ...
    } else if (persistent.damageApplied && effect.script === "starryFormChalice") {
  ...
  function drawPurpleLines() {
    ...
    if (isSableEncounter() && state.sableFinalePurpleLines) {
  ...
  function drawSoul() {
    ...
    if (state.sableAssetMismatchActive) {
      if (state.sableAssetMismatchSwitchFrames > 0) {
        drawAssetMismatchSwitchGlitch(s.x, s.y, state.sableAssetMismatchSwitchFrames);
      }
      drawBullet({
        x: s.x,
        y: s.y,
        ...
        type: "glitchOrb",
        angle: 0,
        age: Math.max(0, state.enemyTimer),
        alpha: 1,
        harmless: false,
        glitchPink: "#ff2bd6",
        glitchBlue: "#22d8ff",
        glitchPinkSeed: 0.37,
        glitchBlueSeed: 0.71,
        glitchPixelSeed: 0.53
      });
      return;
    }

    if (state.attackType === ATTACK_TYPE.PURPLE) {
      if (state.sableGlitchedSoul) {
        drawGlitchedHeart(s.x, s.y, "#9d5cff");
      } else {
    ...
    if (state.attackType === ATTACK_TYPE.BLUE) {
      if (state.sableGlitchedSoul) {
        drawGlitchedHeart(s.x, s.y, "#39a7ff");
      } else {
    ...
    if (state.attackType === ATTACK_TYPE.GREEN) {
      if (state.sableGlitchedSoul) {
        drawGlitchedHeart(s.x, s.y, "#25d65f");
      } else {
    ...
    if (state.sableGlitchedSoul) {
      drawGlitchedHeart(s.x, s.y, "#ff1e35", true);
      return;
    }
  ...
  function drawGlitchedHeart(x, y, baseColor, useRedSprite = false) {
    ...
    const transitionBoost = state.sableSoulModeTransition ? 1.8 : 1;
    ...
    const fragmentCount = state.sableSoulModeTransition ? 12 : 7;
  ...
  function drawSableShieldGlitchOut() {
    const glitch = state.sableShieldGlitchOut;
    if (!glitch) return;
  ...
  function drawBullet(b) {
    ...
    if (b.assetMismatchProjectile && state.sableAssetMismatchActive) {
      ctx.rotate(-b.angle);
      ctx.scale(0.72, 0.72);
    ...
    if (b.type === "glitchOrb") {
    ...
    } else if (b.type === "blueLine" || b.type === "orangeLine") {
      ...
      if (b.glitchy) drawSableGeometryGlitchFragments(b, b.width, b.height, true);
      ...
      ctx.fillRect(-b.width / 2, 0, b.width, b.height);
      ctx.strokeRect(-b.width / 2, 0, b.width, b.height);
      if (b.glitchy) drawSableGeometryGlitchFragments(b, b.width, b.height, false);
