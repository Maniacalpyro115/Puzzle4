(() => {
  ...
  function nextBattleDialog() {
    const battleDialog = enemyData.battleDialog;
    const fallback = `* ${enemyData.name} refuses to die.`;
  ...
  function resolveAttack() {
    ...
    setTimeout(() => {
      if (state.enemyHP <= 0) {
        ...
        state.message = enemyData.winMessage;
      } else {
  ...
  function beginEnemyAttack() {
    ...
    state.soul.x = state.box.x + state.box.w / 2;
    state.soul.y = state.box.y + state.box.h / 2;
    state.message = enemyData.enemyWarmupMessage;
  }
  ...
  function useMenuSelection() {
    ...
    if (item === "ACT") {
      state.phase = PHASE.MESSAGE;
      state.message = enemyData.actMessage;
      state.textTimer = 0;
    ...
    if (item === "MERCY") {
      state.phase = PHASE.MESSAGE;
      state.message = state.enemyHP < enemyData.mercyLowHpThreshold
        ? enemyData.mercyLowHpMessage
        : enemyData.mercyHighHpMessage;
      state.textTimer = 0;
  ...
  function updateEnemyAttack() {
    ...
    state.message = enemyData.enemyAttackMessage;

    if (Array.isArray(enemyData.attackPatterns)) {
      const attackPattern = enemyData.attackPatterns[state.pattern];
  ...
  function drawEnemy() {
    ...
    if (sprites.enemy.ready) {
      ctx.drawImage(sprites.enemy, cx - 100, cy - 120, 200, 200);
    } else {
      ctx.save();
      ctx.translate(cx, cy);
      ctx.fillStyle = "#111";
