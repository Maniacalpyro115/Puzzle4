(() => {
  ...
  function beginAttack() {
    ...
    markEnemyLeaveAfterAction("FIGHT");
  ...
  function finishDamageResult() {
    ...
    if (consumeEnemyLeaveAfterAction()) {
      beginEnemyLeaveFromAction();
      return;
    }
  ...
  function beginDefenseBoxMorph() {
    ...
    beginBoxMorph(defenseBox, PHASE.ENEMY, () => {
      state.soul.x = state.box.x + state.box.w / 2;
      state.soul.lane = Math.floor(getPurpleLineCount() / 2);
      state.soul.vy = 0;
  ...
  function markEnemyLeaveAfterAction(item) {
    const bossData = currentBossData();
    state.leaveAfterPlayerAction = bossData.leaveOnNonAct === true && item !== "ACT";
  }

  function consumeEnemyLeaveAfterAction() {
    const shouldLeave = state.leaveAfterPlayerAction;
    state.leaveAfterPlayerAction = false;
    return shouldLeave;
  }
  ...
  function beginEnemyLeaveFromAction() {
    const bossData = currentBossData();

    stopCurrentMusic();
    state.phase = PHASE.MESSAGE;
    state.message = typeof bossData.leaveMessage === "string"
      ? bossData.leaveMessage
      : "* The enemy leaves.";
    state.textTimer = 0;
    state.bullets = [];
    state.attack.active = false;
    state.leaveAfterPlayerAction = false;
    state.enemyDialogMessage = "";
    setTimeout(beginDefeatDissolve, 1800);
  }
  ...
  function useSelectedItem() {
    ...
    markEnemyLeaveAfterAction("ITEM");
    ...
    advanceBattleDialog();
    setTimeout(continueAfterPlayerAction, 3250);
  }
  ...
  function resolveMercy() {
    ...
    markEnemyLeaveAfterAction("MERCY");
  ...
  function getPurpleLineYs() {
    const box = state.box;
    const lineCount = getPurpleLineCount();

    if (lineCount <= 1) return [box.y + box.h * 0.5];

    return Array.from({ length: lineCount }, (_, index) => (
      box.y + box.h * (0.16 + (0.68 * index) / (lineCount - 1))
    ));
  }

  function getPurpleLineCount() {
    const bossData = currentBossData();
    const count = Number.isInteger(bossData.purpleLineCount)
      ? bossData.purpleLineCount
      : PURPLE_LINE_COUNT;
  ...
  function drawBullet(b) {
    ...
    if (b.type === "blueLine" || b.type === "orangeLine") {
      ...
      const stem = Number.isFinite(b.stem) ? b.stem : b.r * 3.2;

      ctx.fillStyle = b.color || "#fff";
      ctx.strokeStyle = b.color || "#fff";
      ctx.lineWidth = 3;
      ctx.beginPath();
      ctx.ellipse(-b.r * 0.35, b.r * 0.7, b.r * 0.9, b.r * 0.62, -0.35, 0, Math.PI * 2);
      ctx.fill();
      ctx.beginPath();
      ctx.moveTo(b.r * 0.35, b.r * 0.62);
      ctx.lineTo(b.r * 0.35, b.r * 0.62 - stem);
      ctx.stroke();
      ctx.beginPath();
      ctx.moveTo(b.r * 0.35, b.r * 0.62 - stem);
      ctx.quadraticCurveTo(b.r * 1.7, b.r * 0.2 - stem, b.r * 1.45, b.r * 1.15 - stem);
      ctx.stroke();
